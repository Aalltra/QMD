...
async function viewListing(listingId) {
    try {
        ...
        contactSellerBtn.addEventListener('click', () => {
            requireAuth(() => {
                showNotification('Contact feature not implemented in this demo', 'info');
            });
        });
...
async function createNewListing() {
    try {
        ...
        if (!title || !categoryId || isNaN(price) || !description) {
        ...
        await API.createListing(user.id, categoryId, componentData, price, condition);
